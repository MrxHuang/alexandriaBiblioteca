  ...
  q += toward * uMouseInfluence * 0.2;
    ...
    if (uColorCount > 0) {
      ...
      for (int i = 0; i < MAX_COLORS; ++i) {
            ...
            // Aumentar intensidad de los colores para hacerlos más vibrantes
            w = pow(w, 0.7); // Hace que los colores sean más intensos
            sumCol += uColors[i] * w;
            cover = max(cover, w);
      }
      ...
      a = uTransparent > 0 ? cover : 1.0;
        ...
        for (int k = 0; k < 3; ++k) {
            ...
            float m = mix(m0, m1, kMix);
            col[k] = pow(1.0 - exp(-6.0 / exp(6.0 * m)), 0.7); // Aumentar intensidad
        }
    ...
    // Mejorar la intensidad cuando es transparente - evitar atenuación excesiva
    vec3 rgb = (uTransparent > 0) ? col * (0.7 + 0.3 * a) : col;
    // Aumentar intensidad y saturación de los colores para hacerlos más vibrantes
    rgb = pow(rgb, vec3(0.85)); // Gamma correction que hace los colores más brillantes
